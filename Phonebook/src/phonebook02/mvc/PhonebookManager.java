...
import java.util.Arrays;import javax.swing.text.PlainDocument;
...
public class PhonebookManager implements PhonebookController {
	...
	public int insert(String name, String phoneNum, String memo) {
		...
		pbList[count].setUid(getMaxUid()+1);	//기존 최대uid값 보다 1증가한값 (중복값이 나오지않게하기위함)
	...
	for (int index = 0; index < count; index++) {
		if (pbList[index].getUid() == uid) {
			return pbList[index]; // uid값을 발견하면 리턴하겠습니다
		}
	}	
	...
	public int updateByUid(int uid, String name, String phoneNum, String memo) {
		...
		//매개변수 검증
		if (uid < 1) return result;			// 고유저장값이있는지
		if (name == null || name.trim().length() == 0) return result;	//이름이 들어가있는지
