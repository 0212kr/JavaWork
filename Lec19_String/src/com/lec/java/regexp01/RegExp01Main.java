...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		regex = "My....";				// My로 시작하고 뒤에 임의의 문자 4개의 패턴
		...
		matcher = pat.matcher(input);	// 패턴 매칭의 결과 Matcher 생성
		...
		// My.... 까지 매칭후 다시같은값으로 돌릴경우 찾은값 이후부터 매칭하기때문에 실패가나옴
		
		
		
		// reset() 다시 처음부터 패턴매칭하도록 reset 함.
		matcher = matcher.reset();					// 매처의값을 다시 초기입력상태로 바꿈
		if (matcher.find()) {
			System.out.println("find() 성공");
			System.out.println(matcher.group());
		...
		System.out.println(matcher.replaceFirst("XXXX"));	//	처음 발견된 패턴을 XXXX로 변경
		...
		if(Pattern.matches("My....", "Myabcd")) {
			System.out.println("Pattern.matched() 매칭OK");
		}else {			
			System.out.println("Pattern.matched() 매칭 Fall");
		}
